...
const ModelSelector: React.FC<ModelSelectorProps> = ({ model, onModelChange, disabled }) => {
  return (
    <Select value={model} onValueChange={(value) => onModelChange(value as Models)} disabled={disabled}>
      ...
      <SelectContent>
        {Object.values(Models).map((modelValue) => (
          <SelectItem key={modelValue} value={modelValue} className='flex items-center gap-2'>
            <div className='flex items-center gap-2'>
              <Image src={modelLogos[modelValue]} alt={modelValue} width={16} height={16} className='rounded-sm' />
