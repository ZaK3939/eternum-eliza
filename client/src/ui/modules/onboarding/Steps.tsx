...
import { OnboardingButton } from '../../layouts/OnboardingButton';
...
export const SettleRealm = ({ onPrevious }: { onPrevious: () => void }) => {
  ...
  // seasonPassElements のマッピング部分を修正
  const seasonPassElements = useMemo(() => {
    return seasonPassRealms.reduce<JSX.Element[]>((elements, seasonPassRealm, index, array) => {
      if (index % 2 === 0) {
        const nextRealm = array[index + 1];
        elements.push(
          <div key={seasonPassRealm.realmId} className='grid grid-cols-2 gap-3'>
            <SeasonPassRealm
              seasonPassRealm={seasonPassRealm}
              selected={selectedRealms.includes(seasonPassRealm.realmId)}
              setSelected={(selected: boolean) =>
                setSelectedRealms(
                  selected
                    ? [...selectedRealms, seasonPassRealm.realmId]
                    : selectedRealms.filter((id) => id !== seasonPassRealm.realmId),
                )
              }
            />
            {nextRealm && (
              <SeasonPassRealm
                seasonPassRealm={nextRealm}
                selected={selectedRealms.includes(nextRealm.realmId)}
                setSelected={(selected: boolean) =>
                  setSelectedRealms(
                    selected
                      ? [...selectedRealms, nextRealm.realmId]
                      : selectedRealms.filter((id) => id !== nextRealm.realmId),
                  )
                }
              />
            )}
